...
@router.get("/process-map", response_model=ProcessMapResponse)
async def process_map(
    ...
    """Граф процесса: режим top_paths (объединение топ-N путей с панелью путей)
    или frequency (DFG с порогом частоты рёбер и лимитом узлов)."""
    virtual = await _get_vd(db, project_id, vd_id)
    df = await analytics_service.load_vd_dataframe(
        db, virtual, analytics_service.filter_from_query(filters)
    )
    ...
    if mode == "top_paths":
        variants_df = get_top_n_variants(df, n=n, activity_col=column)
        graph = build_top_paths_graph(df, variants_df, column)
        paths = [
            PathRow(
                index=idx,
                trace=list(row["trace"]),
    ...
    return OperationsResponse(
        items=[
            OperationSummaryRow(
                ...
                median_own_duration_seconds=float(row["median_own_duration_seconds"]),
                avg_share_pct=float(row["avg_share_pct"]),
            )
...
async def filter_options(
    ...
    """Уникальные значения для селектов панели фильтров."""
    virtual = await _get_vd(db, project_id, vd_id)
    ...
    return FilterOptionsResponse(
        ...
        roles=_distinct("role"),
        resources=_distinct("resource"),
        activities=_distinct("activity"),
    )
